...
- **`list` rendered failure memories as `[object Object]`.** Failure rows store a structured payload (`{ what_failed, why_failed, what_should_do }`) — sometimes as an object, sometimes as a JSON string nested under `content` — and the preview formatter stringified it naively. It now unwraps nested content/value/text wrappers (parsing stringified-JSON as it descends) and renders failures as a `what_failed → what_should_do` gist.

## [0.36.1] - 2026-07-22

### Added

- **`claude-recall list` command.** Enumerate stored memories with no query — the gap between `search` (needs a query, returns a ranked top-N) and `stats` (counts only, no content). Lists everything in scope, newest first, showing each row's numeric id, type, active state, timestamp, and a content preview. Filters: `-t/--type`, `-l/--limit` (default 20), `--all`, `--project <id>`, `--global`, `--json`. Ids and keys map straight to `delete <key>` / `rules promote <id>`.
...
- README: new "Why not just steering files or CLAUDE.md?" section — the positioning answer in five bullets (documentation vs. memory).
...
- The hook dispatcher logs its scope decision once per invocation — `scope [hooks]: payload.cwd=… process.cwd=… pin=… → project=…` in `hook-logs/hook-dispatcher.log`. Makes "which project did this hook scope to, and why" answerable from the log, which is the definitive check for `kiro --resume` scoping questions (it shows whether the runtime-declared cwd differs from the inherited one).
